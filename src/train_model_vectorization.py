...
from model_vectorization import K_nearest_Neighbors_Vectoarization
from read_data import split_data

def train(x_train,y_train):
    model=K_nearest_Neighbors_Vectoarization(k_n_neighbors=5,distance_func="Euclidean")
    model.fit(x_train,y_train)
    return model
    
def main():
    x_train,x_test,y_train,y_test=split_data()  #split data
    ################ Scaler data #################
    ...
    ################# Model test #################
    y_test_pred=model.predict_vectorization(x_test,batch_size=10)
    end_time=time.time()
    ############## Model accuracy ################
    accuracy=model.accuracy_vectorization(y_test,y_test_pred)
